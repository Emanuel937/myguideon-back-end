...
router.put('/add/:id', async (req, res) => {
    ...
    const selectQuery = 'SELECT culture FROM destination WHERE id = ?';
    const updateQuery = 'UPDATE destination SET culture = ? WHERE id = ?';
    ...
    try {
        ...
        if (section.toLowerCase().includes('brief')) {
            data[0] = content; // Mettre à jour la section brief
        } else if (section.toLowerCase().includes('language')) {
            data[1] = content; // Mettre à jour la section language
        } else if (section.toLowerCase().includes('religion')) {
...
router.get('/:id',  async(req, res)=>{
    ...
    const query         = 'SELECT culture FROM destination WHERE id = ?';
