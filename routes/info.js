...
router.put('/add/:id', async (req, res) => {
    ...
    const selectQuery = 'SELECT info FROM destination WHERE id = ?';
    const updateQuery = 'UPDATE destination SET info = ? WHERE id = ?';
    ...
    try {
        ...
        if (section.toLowerCase().includes('travel')) {
            data[0] = content; // Mettre à jour la section brief
        } else if (section.toLowerCase().includes('applications')) {
            data[1] = content; // Mettre à jour la section language
        } else if (section.toLowerCase().includes('transportation')) {
            data[2] = content // Mettre à jour la section religion
        }
...
router.get('/:id',  async(req, res)=>{
    ...
    const query         = 'SELECT info FROM destination WHERE id = ?';
